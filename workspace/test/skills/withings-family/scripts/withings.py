...
def _base_dir() -> Path:
    ...
    legacy = Path.home() / ".config" / "withings-family"
...
def get_workouts(user_id='default', days=1):
    """Fetch recent workouts (runs, etc)."""
    token = get_valid_token(user_id)
    end_date = datetime.now()
    start_date = end_date - timedelta(days=days)
    data = post_request('/v2/measure', {
    ...
    # Logic to de-duplicate near-identical start times
    unique_workouts = {}
    for w in data.get('series', []):
        unique_workouts[w['startdate']] = w
        
    results = []
    for w in unique_workouts.values():
        d = w.get('data', {})
        duration_min = (w['enddate'] - w['startdate']) // 60
        results.append({
            'date': w['date'],
            'start': datetime.fromtimestamp(w['startdate']).strftime('%H:%M'),
            'duration_min': duration_min,
            'calories': d.get('manual_calories') or d.get('calories', 0),
            'distance_km': d.get('manual_distance') or d.get('distance', 0) / 1000
        })
...
def main():
    ...
    if command == 'auth':
        ...
        get_workouts(user_id, days)
    elif command == 'sleep':
        days = int(params[0]) if params else 7
        get_sleep(user_id, days)
    else:
        print("Usage: python3 withings.py [userId] <command> [params]")
        print("Commands: auth [code], weight, body [limit|all], activity [days], sleep [days], workouts [days]")
        print("\nExamples:")
